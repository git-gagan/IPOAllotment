...
async function processAllocationDictU(
    allocation_dict,
    lotSize,
    totalSize,
    ipo_id,
    ...
    */
    ...
    for (let i in allocation_dict) {
        ...
        if (
            !(
                statusInfo &&
                allocation_dict[i]["investor_type_id"] in statusInfo
            )
        ) {
        ...
        if (
            !(
                allocation_dict[i]["investor_id"] in
                processed_dict["investorInfo"]
            )
        ) {
            let dmat_info = await dematFromDb(
                allocation_dict[i]["investor_id"],
                ipo_id
            );
    ...
    console.log("Processed Dict U = ", processed_dict);
    console.log("Status Info U = ", statusInfo);
    return processed_dict;
}

async function processAllocationDictO(
    allocation_dict,
    lotSize,
    totalSize,
    ipo_id,
    ...
    */
    ...
    for (let i in allocation_dict) {
        ...
        console.log("Investor Type:- ", allocation_dict[i]["investor_type_id"]);
        let available_shares =
            statusInfo[allocation_dict[i]["investor_type_id"]][
                "sharesMaxLimit"
            ] -
            statusInfo[allocation_dict[i]["investor_type_id"]][
                "sharesAllotted"
        ...
        if (available_shares <= 0) {
            console.log(
                "No more shares available for investor type:- ",
                allocation_dict[i]["investor_type"]
            );
            continue;
        }
        if (allotment_principle == 2) {
            ...
            bid_price =
                ...
                statusInfo[allocation_dict[i]["investor_type_id"]]["sharesBid"];
        }
        if (
            !(
                allocation_dict[i]["investor_id"] in
        ...
        let expected_investment_amount =
            processed_dict["investorInfo"][allocation_dict[i]["investor_id"]][
                "amount_invested"
            ] +
            shares_demanded * bid_price;
        if (
            expected_investment_amount >=
            issuer_info[ipo_id]["userInfo"][allocation_dict[i]["investor_id"]][
                "total_invested"
            ]
        ) {
            // If actual amount invested is less than what is getting allotted to the investor
            console.log(
                "Investor has't invested this much!!! Allot less OR None\n"
            );
            let money_balance =
                issuer_info[ipo_id]["userInfo"][
                    allocation_dict[i]["investor_id"]
                ]["total_invested"] -
                processed_dict["investorInfo"][
                    allocation_dict[i]["investor_id"]
            ...
            shares_demanded =
                Math.floor(money_balance / (lotSize * bid_price)) * lotSize;
        }
        if (available_shares >= shares_demanded) {
            // If shares available are more than the investors current demand
            shares_to_be_allotted = shares_demanded;
        ...
        processed_dict["totalAmount"] += amount_invested;
        processed_dict["totalShares"] += shares_to_be_allotted;
        statusInfo[allocation_dict[i]["investor_type_id"]]["sharesAllotted"] +=
            shares_to_be_allotted;
    }
...
async function processAllocationDictOR(
    allocation_dict,
    lotSize,
    totalSize,
    ipo_id,
    ...
    for (let key in temp_investor_dict) {
        ...
        if (diff_shares != 0) {
            ...
            if (diff_shares >= lotSize / 2) {
                ...
                shares_demanded =
                    Math.floor((shares_demanded + lotSize) / lotSize) * lotSize;
            } else {
                // Round off to lower side
                shares_demanded =
                    Math.floor(shares_demanded / lotSize) * lotSize;
            }
        }
        if (!(key in processed_dict["investorInfo"])) {
            ...
            processed_dict["investorInfo"][key]["demat_account"] =
                dmat_info[0]["demat_ac_no"];
        }
        let expected_investment_amount =
            processed_dict["investorInfo"][key]["amount_invested"] +
            shares_demanded * bid_price;
        if (
            expected_investment_amount >=
            issuer_info[ipo_id]["userInfo"][key]["total_invested"]
        ) {
            // If actual amount invested is less than what is getting allotted to the investor
            console.log(
                "Investor has't invested this much!!! Allot less OR None\n"
        ...
        processed_dict["investorInfo"][key]["shares_allotted"] +=
            shares_to_be_allotted;
        processed_dict["investorInfo"][key]["amount_invested"] +=
            amount_invested;
        processed_dict["totalAmount"] += amount_invested;
...
async function getSubscriptionInfo(issuer_info, ipo_id) {
    ...
    for (let i in eligibilityObj) {
        if (!(eligibilityObj[i]["investor_type_id"] in eligibilityObjDict)) {
            eligibilityObjDict[eligibilityObj[i]["investor_type_id"]] = {
                reserve_shares:
                    eligibilityObj[i]["reserve_lots"] *
                    issuer_info[ipo_id]["ipoInfo"]["lot_size"],
            };
        } else {
            // Should never happen
            console.log(
                `${eligibilityObj[i]["investor_type_id"]} exists already!`
            );
        }
    }
    console.log("----------------------");
    ...
    for (let key in usersInfo) {
        ...
        if (!(usersInfo[key]["investor_type_id"] in statusInfo)) {
            ...
            if (isEligible) {
                reserve_shares =
                    eligibilityObjDict[usersInfo[key]["investor_type_id"]][
                        "reserve_shares"
                    ];
            } else {
                reserve_shares = 0;
            }
            statusInfo[usersInfo[key]["investor_type_id"]]["sharesMaxLimit"] =
                reserve_shares;
        ...
        statusInfo[usersInfo[key]["investor_type_id"]]["totalInvestment"] +=
            getInvestorInvestment(
                usersInfo[key]["transactions"],
                issuer_info[ipo_id]["ipoInfo"]["lot_size"]
            );
        statusInfo[usersInfo[key]["investor_type_id"]]["sharesBid"] +=
            usersInfo[key]["shares"]["bid"];
        if (
            statusInfo[usersInfo[key]["investor_type_id"]]["sharesBid"] >
            reserve_shares
        ) {
...
export {
    processAllocationDictU,
    processAllocationDictO,
    processAllocationDictOR,
    getSubscriptionInfo,
    segregateStatusDictionary,
    mergeAllocationResultDictionaries,
};
