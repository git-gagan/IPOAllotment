...
async function insertOrUpdateIpo(
    issuer_obj,
    ipo_id,
    update,
    allotment_principle = null,
    fixed_price
) {
    try {
        ...
        if (!update) {
            ...
            values(
                ...
                '${issuer_obj[ipo_id]["ipoInfo"]["ipo_announcement_date"]
                    .toISOString()
                    .split("T")
                    .join(" ")}',
                '${issuer_obj[ipo_id]["ipoInfo"]["bid_start_date"]
                    .toISOString()
                    .split("T")
                    .join(" ")}',
                '${allotment_principle}',
...
async function getInvestorTypeAllocation(ipo_id) {
    try {
        ...
        console.error(
            `Failed to get po investor_type allocation info: ${error}`
        );
    }
}
