...
async function registerUserIssuer(username) {
    try {
        ...
        const adminIdentity = await wallet.get('admin-issuer');
        if (!adminIdentity) {
            console.log('An identity for the admin user "admin-issuer" does not exist in the wallet');
        ...
        const adminUser = await provider.getUserContext(adminIdentity, 'admin-issuer');
