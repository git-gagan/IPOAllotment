...
export const issuerDashboard = async (req, res) => {
  ...
  let ipoInfo = await getIpoInfo(req.user.user_id);
  console.log("IPOINFO", ipoInfo);
  var date = new Date(ipoInfo.bid_start_date);
  var endDate = new Date(ipoInfo.bid_time);
  ...
  const formattedEndDate = enddd + "-" + endmm + "-" + endyyyy;
  console.log(formattedEndDate);
  let allotmentPrinciple = await getAllotmentPrinciple(
    ipoInfo.allotment_principle
  );
  let ipoBucket = await getIpoBucket(req.user.user_id);
  let investorClassification = await getInvestorClassification(
    req.user.user_id
  );
  let investor_type_allocation_info = await getInvestorTypeAllocation(
    req.user.user_id
  );
...
export const postLaunchIpo = async (req, res) => {
  ...
  console.log(req.user.user_id);
  if (ipoInfo) {
    return res.render("issuer-dashboard.jade", {
      session: req.user.user_name,
      role_id: req.user.role_id,
      ipoInfo: ipoInfo,
      bid_start_date: formattedDate,
      ...
      investorClassification: investorClassification,
      allotment_info: investor_type_allocation_info,
      message: `The IPO for Issue ${ipoInfo.ticker} has been successfully registered. Launch at ${formattedDate}`,
    });
  }
  // Some error occured
...
export const updateIssuer = async (req, res) => {
  ...
  let query = "";
  try {
    let updateIpo = await updateIpoIdentifiers(
      req.user.user_name,
      isin,
      cusip,
      ticker
