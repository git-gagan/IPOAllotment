  ...
  db.get(
    ...
    async (err, row) => {
      if (err) {
        ...
        let user_id = uuidv4();
        let insert =
          "INSERT INTO tbl_user (user_id,user_name, user_pwd,full_name) VALUES (?,?,?,?)";
...
export const postRegisterInvestorStep3 = async (req, res) => {
  ...
  let insert_investor_info =
    "INSERT INTO tbl_investor_info (investor_id,investor_type, pan,investor_name,bank_account_no,ifsc_code,lei,swift_address) VALUES (?,?,?,?,?,?,?,?)";
  let insert_investor_dmat =
    "INSERT INTO tbl_investor_dmat (investor_id,demat_ac_no,dp_id) values(?,?,?)";

  let firstQueryError = "";
  db.run(
    insert_investor_info,
    [user_id, investor_type_id, pan, full_name, acctnum, ifsc],
    (err, response) => {
      if (err) {
  ...
  db.run(
    insert_investor_dmat,
    [user_id, dacctnum, dpartid],
    (err, response) => {
      if (err || firstQueryError) {
        ...
        return res.render("login.jade", {
          title: `Investor ${req.query.username}`,
          message: "Successfully registered!!",
          type: "success",
        });
      }
...
export const userInfo = async (req, res) => {
  ...
  db.all(
    `select user_name,lots_bid,bid_amount,is_allotted from tbl_user 
...
export const getRegisterAuthority = (req, res) => {
  res.render("registerauthority.jade", {
    session: req.session.name,
    role_id: role_id,
  });
};
...
export const appliedIpo = async (req, res) => {
  ...
  for (let transaction_no in investor_transactions) {
    if (
      !(
        investor_transactions[transaction_no]["issuer_name"] in
        refined_investor_transactions
      )
    ) {
      refined_investor_transactions[
        investor_transactions[transaction_no]["issuer_name"]
      ] = [];
    }
    refined_investor_transactions[
      investor_transactions[transaction_no]["issuer_name"]
  ...
  res.render("applied-ipo.jade", {
    ...
    role_id: req.user.role_id,
    user: user,
    investor_transactions: refined_investor_transactions,
  });
};
...
export const addDematPost = async (req, res) => {
  ...
  if (addDematRes == 1) {
    ...
    message =
      "Error: Cannot add the dmat. Please try again with valid credentials";
  }
  let user = await getIdFromUsername(req.user.user_name);
  console.log(user);
