...
async function processAllocationDict(allocation_dict, lotSize, totalSize, ipo_id, statusInfo){
    /* 
...
async function getSubscriptionInfo(issuer_info, ipo_id){
    ...
    for(let i in eligibilityObj){
        if (!(eligibilityObj[i]['investor_type_id'] in eligibilityObjDict)){
            eligibilityObjDict[eligibilityObj[i]['investor_type_id']] = {
                "reserve_shares" : (eligibilityObj[i]['reserve_shares_percentage']*issuer_info[ipo_id]['ipoInfo']['totalSize'])/100
            }
        }
        else{
            // Should never happen
    ...
    for (let key in usersInfo){
        ...
        if (!(usersInfo[key]['investor_type_id'] in statusInfo)){
            ...
            statusInfo[usersInfo[key]['investor_type_id']]['sharesMaxLimit'] = eligibilityObjDict[usersInfo[key]['investor_type_id']]['reserve_shares'];
        ...
        if (statusInfo[usersInfo[key]['investor_type_id']]['sharesBid'] > eligibilityObjDict[usersInfo[key]['investor_type_id']]['reserve_shares']){
...
export {processAllocationDict, getSubscriptionInfo};